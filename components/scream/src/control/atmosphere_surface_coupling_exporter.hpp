...
protected:
  ...
  // Actual export routine. If calling in initialize_impl(), set
  ...
  // Dummy Field needed to set up column info for export fields which must be computed
  // (i.e., not a direct 1-1 export from a field in the FieldManager).
  Field dummy_field;
  ...
  // Views storing the vector components and constant multiples for each export
